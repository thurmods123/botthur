...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Coe Pae🐊 @${num.split('@')[0]}\nSeja Bem Vindo, Membro novo tem que mamar.❤️ *${mdata.subject}*`
				...
				teks = `Esse aí foi tarde, Ghost Fdp🐊🐊 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["5564992551969@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Total de Fdp bloqueado* : ${blocked.length}\n*O Naomi estar ativo desde* : ${kyun(uptime)}`
				...
				case 'add':
					...
					if (args.length < 1) return reply('você deseja adicionar sua mãe? Digita o número do cara Fdp🐊')
					...
					try {
						...
						console.log('Error :', e)
						reply('Coloca o nome da pessoa ae Man namoral')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						teks = 'Pedido Recebido, Banidokkkkj :\n'
						...
						mentions(`Admin Tá Bravo, Banidookk🐊 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tmj, Se puder... da uma mamada depois')
